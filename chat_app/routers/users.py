...
from chat_app.users import schemas, repo
...
async def read_users(db: Session = Depends(dependencies.get_db),
                     skip: int = 0,
                     limit: int = 100):
    return repo.get_users(db=db, skip=skip, limit=limit)
...
async def read_user(*,
    ...
    db_user = repo.get_user(db=db, id=user_id)
    if db_user is None:
...
async def create_user(*,
                      ...
                      db: Session = Depends(dependencies.get_db)):
    db_user = repo.get_user_by_email(db=db, email=user.email)
    if db_user:
        raise HTTPException(
